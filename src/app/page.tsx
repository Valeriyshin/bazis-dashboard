...
const ZHK_CLASS_SEED: Record<string, string> = {
  ...
  [normz("Vesper")]: "Бизнес",
  [normz("Neopark")]: "Комфорт",
  [normz("Horizon")]: "Комфорт",
  [normz("Riviera Plus")]: "Комфорт",
  [normz("Alter")]: "Комфорт",
  [normz("Fortis")]: "Комфорт",
  [normz("Adamant Life")]: "Комфорт",
  [normz("Adamant Plus")]: "Комфорт",
  [normz("Tribeca")]: "Бизнес",
  // Строки-разрезки по городам (сквозные акции) — "·" и пробелы normz() убирает
  // одинаково, поэтому ключ можно писать и без city-разделителя.
  [normz("Летние скидки Шымкент")]: "Комфорт",
  [normz("Летние скидки Атырау")]: "Комфорт",
  [normz("Коммерция Алматы")]: "Коммерция",
  [normz("Коммерция Астана")]: "Коммерция",
  [normz("Коммерция Шымкент")]: "Коммерция",
  [normz("Коммерция Атырау")]: "Коммерция",
  [normz("Коммерция Караганда")]: "Коммерция",
  [normz("Коммерция Актобе")]: "Коммерция",
};
...
const NON_ZHK = new Set([
  ...
  "ThruPlay", "Thruplay", "Просмотры Thruplay", "Просмотры", "Охват SMM", "SMM", "SMM Пост", "Трафик SMM", "Вовлеченность SMM",
        ...
        {dbAgg && (
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div className="muted" style={{ fontSize: 12, marginBottom: 10 }}>
                Ручное поле «Источник информации» проверяется по полю «Описание» карточки: если там есть цепочка
                кампании без UTM-меток (формат Ads Manager) → Instagram, UTM-метка сайта → «Сайт жилого комплекса»,
                другие UTM-источники — по значению метки. Проверка идёт по всем карточкам, а не только по
                «Реклама в интернете»/пустым — если в «Описании» есть сигнал, он приоритетнее ручной пометки.
                Значение оператора остаётся только там, где в «Описании» ничего не нашлось.
              </div>
              <div className="table-scroll">
                <table>
